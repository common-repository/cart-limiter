...
trait NoticeUtils {
	...
	public static function notice_assets() {
		wp_enqueue_style( 'gpls-general-module-notice-styles', static::$plugin_info['url'] . '/includes/utils/notice/notice.min.css', array(), static::$plugin_info['version'], 'all' );
	}
	...
	public static function error_message( $messages, $return = true, $html = '', $include_close_btn = true, $buttons = array() ) {
		...
		<div class="<?php echo esc_attr( static::$plugin_info['classes_general'] . '-error-notice' ); ?> <?php echo esc_attr( static::$plugin_info['classes_general'] . '-notice' ); ?> position-relative">
			...
			<ul class="errors-list notices-list">
				...
				<li><?php printf( esc_html__( '%s', '%s' ), $message, static::$plugin_info['name'] ); ?></li>
				<?php endforeach; ?>
			</ul>
			<?php if ( ! empty( $html ) ) {
				wp_kses_post( $html );
	...
	public static function ajax_request_handle( $nonce_key, $cap = 'administrator', $error_messages = array(), $codes = array(), $context = '' ) {
		if ( empty( $_POST['nonce'] ) ) {
			wp_send_json_error( printf( esc_html__( '%s', '%s' ), ! empty( $error_messages['nonce'] ) ? $error_messages['nonce'] : 'Sorry, you are not allowed to perform this action.', static::$plugin_info['name'] ), ! empty( $codes['nonce'] ) ? $codes['nonce'] : 403 );
		}
		...
		if ( ! current_user_can( $cap ) ) {
			self::ajax_response( ! empty( $error_messages['cap'] ) ? $error_messages['cap'] : 'Sorry, you are not allowed to perform this action.', 'error', ! empty( $codes['cap'] ) ? $codes['cap'] : null, $context );
		}
	}
}
