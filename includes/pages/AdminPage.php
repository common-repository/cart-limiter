<?php
namespace GPLSCore\GPLS_PLUGIN_WWCLR\pages;
...
abstract class AdminPage extends Base {
	...
	public function add_page() {
		...
		if ( $this->page_props['is_woocommerce'] ) {
			...
			if ( ! empty( $this->page_props['hide_save_button'] ) ) {
				add_action( self::$plugin_info['name'] . '-' . $this->page_props['menu_slug'] . '-page-content', array( $this, 'hide_woo_save_btn' ), 100, 2 );
			}
			return;
		}
	...
	/**
	 * Hide Woo Settings Page Save Button.
	 *
	 * @return void
	 */
	public function hide_woo_save_btn() {
	...
	public function woo_settings_tab_action() {
		$this->page_output_function( false );
		do_action( self::$plugin_info['name'] . '-' . $this->page_props['menu_slug'] . '-woo-main-settings-tabs-action' );
	}
	...
	<nav class="nav-tab-wrapper woo-nav-tab-wrapper wp-clearfix">
		...
		foreach ( $this->tabs as $tab_name => $tab_arr ) :
			...
			?>
		<a href="<?php echo esc_url( $this->page_path . '&' . $this->page_props['tab_key'] . '=' . esc_attr( $tab_name ) ); ?>" class="<?php echo esc_attr( $classname ); ?>"><?php printf( esc_html__( '%s', '%s' ), $tab_arr['title'], self::$plugin_info['name'] ); ?></a>
		<?php endforeach; ?>
	</nav>
		<?php
	}
	...
	public function page_output_function( $display_tabs = true, $show_notices = true ) {
		...
		<div class="wrap <?php echo esc_attr( self::$plugin_info['classes_prefix'] . '-page-template-wrapper gpls-general-page-template-wrapper' ); ?> mt-0 bg-light p-3 mt-5 min-vh-100">
			...
			<!-- Template Title -->
			<?php if ( empty( $tab_data['hide_title'] ) ) : ?>
				<h1 class="wp-heading-inline mb-4 shadow-sm p-2 bg-white"><?php printf( esc_html( '%s', '%s' ), $tab_data['title'], self::$plugin_info['name'] ); ?></h1>
			<?php endif; ?>
			...
			// Tab's Template.
			if ( ! empty( $this->templates_folder ) && ! empty( $tab_data['template'] ) ) {
				$args = ! empty( $tab_data['args'] ) ? array_merge( $args, $tab_data['args'] ) : $args;
				load_template(
					self::$plugin_info['path'] . 'templates/pages/' . $this->templates_folder . '/' . $tab_data['template'],
					false,
					$args
				);
			}
			...
			// Tab Function.
			if ( $tab && method_exists( $this, $tab . '_tab' ) ) {
				$method_name = $tab . '_tab';
				$this->{$method_name}();
			}

			do_action( self::$plugin_info['name'] . '-' . $this->page_props['menu_slug'] . '-page-content', $this, $tab, $tab_data );
			?>
